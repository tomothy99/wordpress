...
class product_widget_search extends WP_Widget {
	...
	function widget( $args, $instance ) {
		if ( get_integration_type() != 'simple' ) {
			...
			echo apply_filters( 'ic_product_search_form_post_type', '<input type="hidden" name="post_type" value="' . get_current_screen_post_type() . '" />', $instance );
			echo '<div class="search-container"><input class="product-search-box" type="search" value="' . get_search_query() . '" id="s" name="s" placeholder="' . $label . '" />
<input class="product-search-submit" type="submit" id="searchsubmit" value="' . $search_button_text . '" /></div>
</form>';
