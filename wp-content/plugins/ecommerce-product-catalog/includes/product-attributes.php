	...
	if ( $max_attr > 0 ) {
		...
		for ( $i = 1; $i <= $max_attr; $i++ ) {
			...
			if ( !empty( $product_meta[ '_attribute-label' . $i ] ) ) {
				$label = is_array( $product_meta[ '_attribute-label' . $i ] ) ? $product_meta[ '_attribute-label' . $i ][ 0 ] : $product_meta[ '_attribute-label' . $i ];
				if ( !empty( $label ) ) {
					$label_id	 = ic_add_product_attribute_label( $label );
					$attr_ids[]	 = $label_id;
					$value		 = is_array( $product_meta[ '_attribute' . $i ] ) ? $product_meta[ '_attribute' . $i ][ 0 ] : $product_meta[ '_attribute' . $i ];
